...
type RoleMetricGroups = NonNullable<
  MultiPlayerComparisonResponse["role_metrics"]
>;

type MultiPlayerRoleMetricsProps = Readonly<{
  ...
  variant?: "target" | "pair" | "all_players";
}>;
...
function RoleMetricValue({
  ...
  locale: string;
  unavailable: string;
  accessibleLabel: (total: string, per90: string) => string;
}>) {
  if (
    total === null ||
    total === undefined ||
  ...
  const totalText = formatMetricNumber(
    total,
    Number.isInteger(total) ? 0 : 2,
    locale,
  );

  const per90Text = formatMetricNumber(per90, 2, locale);

  return (
    <span className="font-semibold tabular-nums">
      <span aria-hidden="true">
        {totalText} ({per90Text}/90)
      </span>

      <span className="sr-only">{accessibleLabel(totalText, per90Text)}</span>
    </span>
  );
}
...
export function MultiPlayerRoleMetrics({
  ...
  const t = useTranslations("MultiPlayerRoleMetrics");

  const groupLabels = t.raw("groups") as Readonly<Record<string, string>>;

  const metricLabels = t.raw("metrics") as Readonly<Record<string, string>>;

  const players = [target, ...candidates.map(({ player }) => player)];

  const isPair = variant === "pair";

  const isAllPlayers = variant === "all_players";

  const pairCandidate = candidates[0]?.player;

  const headingId = isPair
    ? "pair-role-metrics-title"
    : isAllPlayers
      ? "all-player-role-metrics-title"
      : "role-metrics-title";
    ...
    >
      <div className="border-b border-border p-5 sm:p-7">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-muted">
          {isPair ? (
            <>
              {t("pair.descriptionBeforeTargetRole")}{" "}
              <span className="font-semibold text-foreground">
                {target.final_role ??
                  target.archetype ??
                  t("pair.targetRoleFallback")}
              </span>{" "}
              {t("pair.descriptionBetweenRoles")}{" "}
              <span className="font-semibold text-foreground">
                {pairCandidate?.final_role ??
                  pairCandidate?.archetype ??
                  t("pair.candidateRoleFallback")}
              </span>
              {t("pair.descriptionAfterRoles")}
            </>
          ) : isAllPlayers ? (
            t("allPlayers.description")
          ) : (
            <>
              {t("descriptionBeforeRole")}{" "}
              <span className="font-semibold text-foreground">
                {target.final_role ?? t("targetRoleFallback")}
              </span>
      ...
      {groups.length === 0 ? (
        <p role="status" className="p-6 text-sm text-muted">
          {isPair
            ? t("pair.unavailableDescription")
            : isAllPlayers
              ? t("allPlayers.unavailableDescription")
        ...
        <div
          ...
          aria-label={
            isPair
              ? t("pair.scrollRegionLabel")
              : isAllPlayers
                ? t("allPlayers.scrollRegionLabel")
                : t("scrollRegionLabel")
          }
          tabIndex={0}
                  ...
                  {group.metrics.map((metric) => (
                    ...
                    >
                      ...
                      {players.map((player) => {
                        ...
                        return (
                          <td key={player.player_id} className="px-5 py-4">
                            <RoleMetricValue
                              total={value?.total}
                              per90={value?.per90}
                              locale={locale}
                              unavailable={t("unavailable")}
                              accessibleLabel={(totalText, per90Text) =>
                                t("valueLabel", {
                                  total: totalText,
                                  per90: per90Text,
                                })
